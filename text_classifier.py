...
def train_head(model, train_dataset, epochs=10, lr=3e-4, device = "cpu"):
    model.train()
...
def finetune(model, train_dataset, epochs=10, lr=3e-4, device = "cpu"):
    model.train()
...
if __name__=="__main__":
    ...
    # training the head
    train_head(model, dl_train, epochs=1, device=device)
    correct, total = test(model, dl_test, device=device)
    print(f"Accuracy after training the head: {correct/total}")
    # finetuning the model
    model = TextClassifier()
    model = model.to(device)
    finetune(model, dl_train, epochs=1, device=device)
    correct, total = test(model, dl_test, device=device)
    print(f"Accuracy after training the head: {correct/total}")
