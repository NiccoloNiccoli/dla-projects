...
if __name__=="__main__":
    ...
    print('Complete')
    plt.figure()
    plt.plot(range(0, num_episodes, 10), cum_rewards)
    plt.xlabel('Episode')
    plt.ylabel('Cumulative reward')
    plt.title('Cumulative rewards')
    # plot_durations(show_result=True)
    # plt.ioff()
    plt.savefig('dqn_lunarlander.png')
    plt.show()

    env_rgb = gym.make('LunarLander-v2', render_mode='rgb_array')
    state, info = env_rgb.reset()
    frames = []
    for t in range(500):
        frames.append(env_rgb.render())
        state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        action = select_action(state)
        state, _,  _, done, _ = env_rgb.step(action.item())
        if done:
            break
    env_rgb.close()
    save_frames_as_gif(frames, path='./', filename='dqn_lunarlander.gif')
