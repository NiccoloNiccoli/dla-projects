...
def parse_args():
    ...
    parser.add_argument('--best-model-save-path', type=str, default='./DRL/car_race')
...
if __name__=="__main__":
    ...
    vec_env = make_vec_env(env_id, n_envs=num_training_envs, seed = args.seed, vec_env_cls = SubprocVecEnv, env_kwargs = {'continuous':False})
    eval_env = make_vec_env(env_id, n_envs = num_eval_envs, seed = args.seed, vec_env_cls = SubprocVecEnv, env_kwargs = {'continuous':False})
    ...
    model.save(f'{args.model_path}/ppo_carracing_{timesteps}')
    ...
    env = model.get_env()
    test_env = make_vec_env(env_id, n_envs = 1, seed = 0, env_kwargs = {'continuous':False}) 
    ...
    imageio.mimsave(f'{args.gif_path}/ppo_carracing_{timesteps}.gif', [np.array(img) for i, img in enumerate(images)], fps=30)
