...
if __name__=="__main__":
    ...
    vec_env = make_vec_env(env_id, n_envs=num_training_envs, seed = args.seed, vec_env_cls = SubprocVecEnv, env_kwargs={'full_action_space' : False})
    eval_env = make_vec_env(env_id, n_envs = num_eval_envs, seed = args.seed, vec_env_cls = SubprocVecEnv, env_kwargs={'full_action_space' : False})
    ...
    if False:
        ...
        model.save(f'{args.model_path}/ppo_froggers_{timesteps}')
    else:
        model = PPO.load(f'{args.model_path}/ppo_froggers_{timesteps}')
    ...
    env = model.get_env()
    test_env = make_vec_env(env_id, n_envs = 1, seed = 0, env_kwargs={'full_action_space' : False}) 
    ...
    imageio.mimsave(f'{args.gif_path}/ppo_froggers_{timesteps}.gif', [np.array(img) for i, img in enumerate(images)], fps=15)